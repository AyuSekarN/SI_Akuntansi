...
if ($method == 'POST') {
    ...
    try {
        ...
        $queryPiutang = "INSERT INTO jurnal_umum (tanggal, kode_akun, akun, debet, kredit, deskripsi) 
                         VALUES ('$tanggal', 102, 'Piutang Usaha', $jumlah, 0.00, 'Piutang Usaha Dari Pendapatan')";
        if (!mysqli_query($conn, $queryPiutang)) {
        ...
        mysqli_commit($conn);
        echo json_encode(["status" => "success", "message" => "Data jurnal kredit berhasil ditambahkan"]);
    } catch (Exception $e) {
